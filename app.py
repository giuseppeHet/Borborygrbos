...
class Player(db.Model):
    ...
    matches_won = db.relationship('Match', backref='winner', lazy=True, foreign_keys='Match.winner_id')
...
# Inizializza tabelle se non esistono (esegui una sola volta)
...
def player_detail(player_id):
    ...
    for deck in decks:
        ...
        deck.colors = ','.join(sorted(color_list))
...
def delete_deck(deck_id):
    ...
    # cancella prima le entries correlate in match_players (opzionale, se usi ON DELETE CASCADE non serve)
...
def add_match():
    ...
    if request.method == 'POST':
        ...
        db.session.flush()  # serve per ottenere l'id della partita
    ...
    # serve per i select del form
    players_list = [dict(id=p.id, name=p.name) for p in players]
    decks_list = [dict(id=d.id, name=d.name, player_id=d.player_id) for d in decks]
    return render_template('add_match.html', players=players_list, decks=decks_list)
